...
export default defineComponent({
  setup(props, { emit }) {
    ...
    const handle = {
      $store,
      ...mapActions('File', ['create', 'delete']),
      ...mapActions(props.storeModel, ['setImage'])
    }
    ...
    const loadImg = () => {
      if (props.id) {
        const avatarUrl = userModuleAvatar(props.id, 'mini')
    ...
    const onDelete = () => {
      if (props.id) {
        handle.delete(props.id).then(res => {
          setImage({})
        })
      }
    }
    ...
    const setImage = row => {
      ...
      handle.setImage({ id: props.moduleId, data }).then(res => {
        message.success('图片更新成功')
        // const avatarUrl = userModuleAvatar(data.id, data.name, 'mini')
        // imageUrl.value = avatarUrl
        // previewVisible.value = true
